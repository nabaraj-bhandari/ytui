...
size_t &scroll_for_focus(Focus f) {
  ...
  case DOWNLOADS:
    return downloads_scroll;
  case SUBSCRIPTIONS:
    return subs_scroll;
  case CHANNEL:
    return channel_scroll;
  case RESULTS:
...
void render_status_bar(const std::vector<Video> *cached_downloads = nullptr) {
  ...
  if (focus == DOWNLOADS) {
    ...
    if (cached_downloads)
      total = static_cast<int>(cached_downloads->size());
    else
      total = static_cast<int>(scan_video_cache().size());
    for (const auto &d : downloads)
      if (d.pid > 0)
        active++;
    info = std::to_string(total) + " files";
    if (active > 0)
      info += " | " + std::to_string(active) + " active";
  }
  if (!info.empty()) {
    attron(A_DIM);
...
void render_video_list_section(int y, int h, const std::string &title,
                               const std::vector<Video> &items, bool active,
                               size_t &offset) {
  int w = getmaxx(stdscr);
  // Mirror thumb_geometry: thumb occupies right (w*35/100) cols
  // so content gets exactly w - thumb_cols = w - (w*35/100)
  int thumb_cols = std::max(8, w * 35 / 100);
  int content_w = thumbnail_shown ? (w - thumb_cols) : w;
  std::string logo = "-=YTUI=-";
  if (active)
    attron(COLOR_PAIR(1) | A_BOLD);
  else
    attron(A_DIM);
  mvprintw(0, (content_w - (int)logo.length()) / 2, "%s", logo.c_str());
  mvprintw(y, 0, "--- %s ", title.c_str());
  for (int i = (int)title.length() + 5; i < content_w; i++)
    addch('-');
  ...
  for (size_t i = 0; i < max_display; i++) {
    ...
    int max_w = content_w - (int)(prefix.length() + num.length()) - 3;
  ...
  if (max_display > 0 && items.size() > max_display) {
    ...
    if (offset > 0)
      mvprintw(top_row, content_w - 2, "^");
    if (offset + max_display < items.size())
      mvprintw(bottom_row, content_w - 2, "v");
  }
}
...
void render_search_view() {
  ...
  int header_x =
      std::max(1, left + std::max(0, (right - left - header_width) / 2));
  if (header_x + header_width >= w)
    header_x = std::max(1, w - header_width - 1);
  int header_y = std::max(0, top - 1);
  attron(A_BOLD | COLOR_PAIR(1));
  mvprintw(header_y, header_x, "%s", header.c_str());
  attroff(A_BOLD | COLOR_PAIR(1));

  std::string help = insert_mode ? "Enter search  Esc normal  Left/Right move"
                                 : "Enter edit  Esc normal  j/k navigate";
  if ((int)help.size() > w - left - 1)
    help.resize(std::max(0, w - left - 1));
  ...
  if (window_width > 0 && query.size() > window_width) {
    if (caret >= window_width)
      window_start = caret - window_width + 1;
    if (window_start + window_width > query.size())
      window_start = query.size() - window_width;
  }
  bool clipped_left = window_start > 0;
  bool clipped_right =
      window_width > 0 && window_start + window_width < query.size();
  std::string display;
  if (window_width > 0) {
    display = query.substr(window_start,
                           std::min(window_width, query.size() - window_start));
    if (display.size() < window_width)
      display.append(window_width - display.size(), ' ');
    if (clipped_left && !display.empty())
      display.front() = '<';
    if (clipped_right && !display.empty())
      display.back() = '>';
  }
  ...
  if (!query.empty() && !display.empty()) {
    ...
    if ((int)placeholder.size() > text_width)
      placeholder.resize(std::max(0, text_width));
    if (highlight_input)
      attron(A_REVERSE);
    attron(A_DIM);
  ...
  if (insert_mode) {
    if (query.empty())
      mvaddch(top + 1, cx, ' ' | A_UNDERLINE | A_BOLD | COLOR_PAIR(1));
    move(top + 1, cx);
    curs_set(1);
  } else {
    curs_set(0);
...
void render_subscriptions_view(int h, int w) {
  ...
  for (int i = (int)title.length() + 5; i < w; ++i)
    addch('-');
  attroff(COLOR_PAIR(1) | A_BOLD);
  ...
  if (subs.size() <= display_count) {
    ...
    if (sel >= subs_scroll + display_count)
      subs_scroll = sel - display_count + 1;
    if (subs_scroll > max_offset)
      subs_scroll = max_offset;
  } else {
    subs_scroll = 0;
  }
  ...
  for (size_t i = 0; i < display_count; ++i) {
    ...
    mvprintw(row_start + (int)i, 2, "%c %2zu. %s", marker, idx + 1,
             name.c_str());
  ...
  if (display_count > 0 && subs.size() > display_count) {
    if (subs_scroll > 0)
      mvprintw(row_start, w - 2, "^");
    if (subs_scroll + display_count < subs.size())
      mvprintw(row_start + (int)display_count - 1, w - 2, "v");
  }
}
...
void enter_subscription_channel(size_t index) {
  ...
  if (subs_cache.size() <= index)
    subs_cache.resize(index + 1);
...
void draw() {
  erase(); // erase() redraws ncurses without sending a terminal clear,
           // so kitty retains the uploaded image in memory across frames.
  int h, w;
  getmaxyx(stdscr, h, w);
  ...
  switch (focus) {
  case HOME: {
    render_video_list_section(1, h - 2, "HISTORY", history, true,
                              history_scroll);
    break;
  }
  case SEARCH:
  ...
  case DOWNLOADS: {
    ...
    render_video_list_section(1, h - 2, "DOWNLOADS", items, true,
                              downloads_scroll);
    break;
  }
  case SUBSCRIPTIONS:
  ...
  case RESULTS:
    ...
    break;
  case CHANNEL: {
    render_video_list_section(1, h - 2, "CHANNEL", channel_videos, true,
                              channel_scroll);
    break;
  }
  }
...
bool handle_input() {
  ...
  auto set_focus = [&](Focus target) {
    ...
    reset_search_state();
    if (!focus_has_video_content(target))
      hide_thumbnail();
  };
  ...
  auto refresh_thumbnail = [&]() {
    ...
    if (focus == CHANNEL) {
      ...
      if (sel >= channel_videos.size())
        sel = channel_videos.size() - 1;
  ...
  const bool navDown =
      (ch == KEY_DOWN) || (allow_vim_nav && (ch == 'j' || ch == 'J'));
  const bool navUp =
      (ch == KEY_UP) || (allow_vim_nav && (ch == 'k' || ch == 'K'));
  const bool navBack = allow_vim_nav && (ch == 'h' || ch == 'H');
  const bool navSelect =
  ...
  auto handle_list_navigation = [&](size_t size,
    ...
    if (move_selection(size))
      return true;
    if (navBack && onBack) {
      onBack();
      return true;
    }
    if (navSelect && onSelect && size > 0 && sel < size) {
      onSelect();
      return true;
    }
    return false;
  ...
  auto handle_video_list =
      [&](const std::vector<Video> &list, const std::function<void()> &onBack,
          const std::function<void()> &onSelect,
        ...
        if (navSelect && onSelect && !list.empty() && sel < list.size()) {
          onSelect();
          return true;
        }
  ...
  if (focus != SEARCH) {
    ...
    if (ch == APP_KEY_SUB_TOGGLE) {
      const Video *v = nullptr;
      if (focus == RESULTS && sel < res.size())
        v = &res[sel];
      else if (focus == HOME && sel < history.size())
        v = &history[sel];
      else if (focus == CHANNEL && sel < channel_videos.size())
        v = &channel_videos[sel];
      else if (focus == DOWNLOADS) {
        const auto &items = ensure_download_items();
        if (sel < items.size())
          v = &items[sel];
    ...
    if (ch == 'r') {
      if (focus == SUBSCRIPTIONS && sel < subs.size()) {
        size_t idx = sel;
        if (subs_cache.size() <= idx)
          subs_cache.resize(idx + 1);
        subs_cache[idx] = fetch_videos(subs[idx].url, MAX_LIST_ITEMS);
        std::string name =
            subs[idx].name.empty() ? subs[idx].url : subs[idx].name;
  ...
  if (focus == SEARCH) {
    ...
    if (!insert_mode) {
      if (navDown) {
        if (search_hist_idx < 0 && !search_hist.empty())
          search_hist_idx = 0;
        else if (search_hist_idx >= 0 &&
                 search_hist_idx + 1 < (int)search_hist.size())
          ++search_hist_idx;
        return true;
      }
      if (navUp) {
        if (search_hist_idx > 0)
          --search_hist_idx;
        else if (search_hist_idx == 0)
          search_hist_idx = -1;
        else if (search_hist_idx < 0 && !search_hist.empty())
          search_hist_idx = (int)search_hist.size() - 1;
        return true;
      }
      if (navSelect) {
      ...
      if (ch == KEY_DC) {
        if (query_pos < query.size())
          query.erase(query_pos, 1);
        return true;
      }
      if (ch >= 32 && ch <= 126) {
        query.insert(query_pos, 1, (char)ch);
        ++query_pos;
        return true;
      }
    }
  ...
  if (focus != SEARCH) {
    auto restore_channel_origin = [&]() {
      reset_search_state();
      Focus target =
          channel_return_active ? channel_return_focus : SUBSCRIPTIONS;
      size_t selection =
          channel_return_active
              ? channel_return_sel
              : (subs_channel_idx >= 0 ? (size_t)subs_channel_idx : 0);
      channel_return_active = false;
      auto clamp = [&](size_t sz) {
        return sz == 0 ? (size_t)0 : std::min(selection, sz - 1);
      };
      subs_channel_idx = -1;
      switch (target) {
      case SUBSCRIPTIONS:
        focus = SUBSCRIPTIONS;
        sel = subs.empty() ? 0 : clamp(subs.size());
        break;
      case RESULTS:
        focus = RESULTS;
        sel = res.empty() ? 0 : clamp(res.size());
        break;
      case DOWNLOADS: {
        focus = DOWNLOADS;
        const auto &items = ensure_download_items();
      ...
      case HOME:
      default:
        focus = (target == HOME ? HOME : target);
        sel = (target == HOME ? 0 : selection);
        break;
      }
      scroll_for_focus(focus) = 0;
      if (focus_has_video_content(focus))
        refresh_thumbnail();
      else
        hide_thumbnail();
    };
    ...
    if (focus == RESULTS) {
      ...
      if (handle_list_navigation(
              ...
              [&] { enter_subscription_channel(sel); }))
        return true;
      list = nullptr;
    }

    if (list && handle_video_list(*list, onBack, onSelect, onDownload))
      return true;
    return true;
  }
